...
async def extract_mlz_info(source_text: str) -> dict:
    ...
    # 1. استخراج محلي دائماً
    local = _extract_info_local(source_text)

    # 2. إن لم يكن هناك مفتاح Gemini نكتفي بالمحلي
    if not get_all_gemini_keys():
        return local

    # 3. طلب Gemini لتحسين أو ملء الفراغات
    ...
    try:
        ...
        if raw:
            ...
            if match:
                ...
                # دمج: Gemini يملأ ما فات المحلي فقط
                for k in ('subject', 'teacher', 'grade', 'year'):
